...
class _Voxelization(Function):
    ...
    def forward(ctx,
        ...
        if max_points == -1 or max_voxels == -1:
            # coors_ = points.new_zeros(size=(points.size(0), 3), dtype=torch.int)
            # dynamic_voxelize(points, coors_, voxel_size, coors_range, 3)

            coors_range = torch.tensor(coors_range).type_as(points).view(1, 6)
            voxel_size = torch.tensor(voxel_size).type_as(points).view(1, 3)
            coors = ((points[:, [2, 1, 0]] - coors_range[:, [2, 1, 0]]) / voxel_size[:, [2, 1, 0]])
            coors = coors.int()
