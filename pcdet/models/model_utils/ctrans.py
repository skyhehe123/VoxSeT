...
class Transformer(nn.Module):
    ...
    def forward(self, src, query_embed, pos_embed, key_padding_mask=None):
        bs, n, c = src.shape
        ...
        memory = self.encoder(src, 
                              ...
                              pos=pos_embed)
        hs = self.decoder(tgt, memory, memory_key_padding_mask=key_padding_mask,
                          pos=pos_embed, query_pos=query_embed)

        return hs.squeeze(), memory.transpose(0,1)
...
def attention(query, key, value, key_padding_mask):
    dim = query.shape[1]
    scores_1 = torch.einsum('bdhn,bdhm->bhnm', query, key) / dim**.5
    scores_2 = torch.einsum('abcd, aced->abcd', key, scores_1)
    scores_2.masked_fill_(key_padding_mask[:, None, None, :], float('-inf'))
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask):
        ...
        query = self.proj_q(query).view(batch_dim, self.dim, self.num_heads, -1)
        key = self.proj_k(key).view(batch_dim, self.dim, self.num_heads, -1)
        value = self.proj_v(value).view(batch_dim, self.dim, self.num_heads, -1)
                             
        x, prob = attention(query, key, value, key_padding_mask)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos).permute(1,2,0),
                                ...
                                value=memory.permute(1,2,0), key_padding_mask=memory_key_padding_mask)
