...
def save_image(frame, cur_save_path, file_idx):
    ...
    for img in frame.images:
        img_path = cur_save_path / ('image_%s/%03d.png' % (str(img.name - 1), file_idx))
        # print(img_path)
        # import pdb; pdb.set_trace()
...
def save_calib(frame, cur_save_path, file_idx):
    ...
    for camera in frame.context.camera_calibrations:
        ...
        T_vehicle_to_cam = T_vehicle_to_cam[:3, :].reshape((12, ))
        Tr_vehicle_to_cam.append([f'{i:e}' for i in T_vehicle_to_cam])
    ...
    for i in range(5):
        calib_context += 'Tr_vehicle_to_cam_' + str(i) + ': ' + \
            ' '.join(Tr_vehicle_to_cam[i]) + '\n'

    with open(cur_save_path / ('calib/%03d.txt' % file_idx), 'w+') as fp_calib:
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, use_two_returns=True):
    ...
    (save_path / sequence_name / 'image_0').mkdir(parents=True, exist_ok=True)
    (save_path / sequence_name / 'image_1').mkdir(parents=True, exist_ok=True)
    (save_path / sequence_name / 'image_2').mkdir(parents=True, exist_ok=True)
    (save_path / sequence_name / 'image_3').mkdir(parents=True, exist_ok=True)
    (save_path / sequence_name / 'image_4').mkdir(parents=True, exist_ok=True)

    (save_path / sequence_name / 'calib').mkdir(parents=True, exist_ok=True)
    sequence_infos = []
    if pkl_file.exists():
    ...
    for cnt, data in enumerate(dataset):
        ...
        save_image(frame, cur_save_dir , cnt)
        save_calib(frame, cur_save_dir, cnt)
